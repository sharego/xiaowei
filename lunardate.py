...
Declare:
    ...
    If you find, welcome to tell me

@author: xiaowei m2xiaowei@163.com
@date: 2013-11-16
'''
...
def lunar_next(lunar):
    'return the next lunar date of lunar'
    ...
    if month == 12 and day >= 29:
        if lunar == lastdate_of_year(year):
            return BasicLunarDate( year+1,1,1,0 )
    leap_month, leap_days = year_leap(year)
    days = days_of_month(year, month)
    if not leap_month or month != leap_month :
        if day < days[0]:
            return BasicLunarDate(year,month,day+1,0)
        else:
            return BasicLunarDate(year,month+1,1,0)
    else: # leap year and month == leap_month
        if leap :
            assert leap_month == month , 'leap month must equal:'+str(lunar)
            if day < leap_days:
                return BasicLunarDate(year,month,day+1,1)
            else:
                return BasicLunarDate(year,month+1,1,0)
        else:
            if day < days[0]:
